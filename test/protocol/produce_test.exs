defmodule Cafex.Protocol.Produce.Test do
  ...
  test "create_request creates a valid payload" do
    ...
    request = Cafex.Protocol.Produce.create_request(1, "foo", "food", 0, "hey", "", 1, 10)
  ...
  test "create_request correctly batches multiple request messages" do
    ...
    request = Cafex.Protocol.Produce.create_request(1, "foo", "food", 0,
      topic: "food", partition: 0, required_acks: 1, timeout: 10, messages: [
        %Cafex.Protocol.Produce.Message{key: "", value: "hey", compression: :none},
        %Cafex.Protocol.Produce.Message{key: "", value: "hi", compression: :none},
        %Cafex.Protocol.Produce.Message{key: "", value: "hello", compression: :none},
      ]
    })
  ...
  test "parse_response correctly parses a valid response with single topic and partition" do
    ...
    expected_response = [%Cafex.Protocol.Produce.Response{partitions: [%{error_code: 0, offset: 10, partition: 0}], topic: "bar"}]
    assert expected_response == Cafex.Protocol.Produce.parse_response(response)
  end
  ...
  test "parse_response correctly parses a valid response with multiple topics and partitions" do
    ...
    expected_response = [%Cafex.Protocol.Produce.Response{partitions: [
        %{error_code: 0, offset: 20, partition: 1},
        %{error_code: 0, offset: 10, partition: 0}
      ], topic: "bar"},
      %Cafex.Protocol.Produce.Response{partitions: [
