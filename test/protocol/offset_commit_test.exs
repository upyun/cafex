defmodule Cafex.Protocol.OffsetCommit.Test do
  ...
  test "create_request creates a valid offset commit message" do
    corr_id = 3
    client_id = "kafka_ex"
    offset_commit_request = %Cafex.Protocol.OffsetCommit.Request{offset: 10, topic: "foo", consumer_group: "bar", metadata: "baz"}
    ...
    request = Cafex.Protocol.OffsetCommit.create_request(corr_id, client_id, offset_commit_request)
  ...
  test "parse_response correctly parses a valid response" do
    ...
    assert Cafex.Protocol.OffsetCommit.parse_response(response) == [%Cafex.Protocol.OffsetCommit.Response{partitions: [0], topic: "food"}]
  end
end
